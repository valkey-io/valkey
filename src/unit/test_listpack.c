...
#ifdef HAVE_AVX512
extern int string2llAVX512(const char *s, unsigned long slen, int64_t *value);

int test_string2llAVX512(int argc, char **argv, int flags) {
    UNUSED(argc);
    UNUSED(argv);
    UNUSED(flags);
    char buf[32];
    ...
    TEST_ASSERT(string2llAVX512(buf, strlen(buf), &v) == 0);
    ...
    valkey_strlcpy(buf, "-1", sizeof(buf));
    TEST_ASSERT(string2llAVX512(buf, strlen(buf), &v) == 1);
    TEST_ASSERT(v == -1);
