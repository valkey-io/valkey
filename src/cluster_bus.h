...
typedef struct clusterBusType {
    ...
    /* Schedule a cluster state recalculation (OK/FAIL) in the next
     * beforeSleep cycle. Called from config.c when local policy (e.g.
     * cluster-require-full-coverage) changes. */
    void (*scheduleUpdateState)(void);
