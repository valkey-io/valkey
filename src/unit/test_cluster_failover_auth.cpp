...
/* A voter that ACKs and then stops being a voting primary in the same
 * election (demoted to replica or drained of its slots) has still cast a vote
 * that was received. The bound must keep it: 1 (V1) + 2 (V4, V5) = 3 meets
 * the quorum after V2 and V3 NACK. Deriving received votes by scanning only
 * current voters would drop V1 and reset a winnable election. */
TEST_F(ClusterFailoverAuthTest, AckThenDemotedVoterKeepsItsVote) {
    clusterProcessFailoverAuthAck(voters[0]);
    EXPECT_EQ(server.cluster->failover_auth_count, 1);

    /* V1 loses its slots; it is no longer a voting primary in our view. */
    voters[0]->numslots = 0;
...
/* A voter gives one response per election. An ACK arriving after that voter
 * already NACKed is contradictory and must be ignored: otherwise the voter is
 * both excluded as a NACK and counted as a vote, inflating the bound. */
TEST_F(ClusterFailoverAuthTest, AckAfterNackFromSameVoterIsIgnored) {
    clusterProcessFailoverAuthNack(voters[0], &nack_msg);
    clusterProcessFailoverAuthAck(voters[0]);
    EXPECT_EQ(server.cluster->failover_auth_count, 0);
    EXPECT_EQ(server.cluster->failover_auth_nack_count, 1);

    /* V2 and V3 NACK: only V4 and V5 remain, 0 + 2 < 3, the election is lost.
     * Had V1's ACK been counted the bound would be 1 + 2 = 3 and the reset
     * would wrongly be skipped. */
    clusterProcessFailoverAuthNack(voters[1], &nack_msg);
    EXPECT_FALSE(electionReset());
    clusterProcessFailoverAuthNack(voters[2], &nack_msg);
    EXPECT_TRUE(electionReset()) << "V1's contradictory ACK was counted as a vote";
}

/* The symmetric contradiction: a NACK arriving after that voter already ACKed
 * must not take the vote away or count as a rejection. */
TEST_F(ClusterFailoverAuthTest, NackAfterAckFromSameVoterIsIgnored) {
    clusterProcessFailoverAuthAck(voters[0]);
    clusterProcessFailoverAuthNack(voters[0], &nack_msg);
    EXPECT_EQ(server.cluster->failover_auth_count, 1);
    EXPECT_EQ(server.cluster->failover_auth_nack_count, 0);
    ...
    /* V2 and V3 NACK: 1 (V1) + 2 (V4, V5) = 3 still meets the quorum. */
    clusterProcessFailoverAuthNack(voters[1], &nack_msg);
    clusterProcessFailoverAuthNack(voters[2], &nack_msg);
    EXPECT_FALSE(electionReset()) << "V1's contradictory NACK displaced its vote";
}

/* A vote only counts while the request it answers is outstanding. After a
 * fast-fail reset, clusterHandleReplicaFailover() arms the retry by setting a
 * new failover_auth_time and clearing failover_auth_count and
 * failover_auth_sent, but failover_auth_epoch keeps the old value until the
 * retry request is sent. A straggling ACK for the abandoned election arriving
 * in that gap must not be counted, or it is carried into the next epoch as a
 * vote nobody cast there. */
TEST_F(ClusterFailoverAuthTest, LateAckDuringRetryDelayIsIgnored) {
    /* Fast-fail reset the election, then the retry was armed. */
    server.cluster->failover_auth_time = ELECTION_TIME + 1000;
    server.cluster->failover_auth_count = 0;
    server.cluster->failover_auth_nack_count = 0;
    server.cluster->failover_auth_sent = 0;

    /* V1's ACK for the abandoned election shows up before the retry request. */
    clusterProcessFailoverAuthAck(voters[0]);
    EXPECT_EQ(server.cluster->failover_auth_count, 0) << "stale ACK counted while no request was outstanding";

    /* The retry request goes out in the next epoch; genuine votes count. */
    server.cluster->failover_auth_epoch = ELECTION_EPOCH + 1;
    server.cluster->failover_auth_sent = 1;
    clusterProcessFailoverAuthAck(voters[0]);
    clusterProcessFailoverAuthAck(voters[1]);
    EXPECT_EQ(server.cluster->failover_auth_count, 2);
}

/* No election at all (failover_auth_time == 0): an ACK is noise. */
TEST_F(ClusterFailoverAuthTest, AckWithNoElectionIsIgnored) {
