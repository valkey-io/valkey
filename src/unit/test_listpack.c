...
int test_listpack(int argc, char **argv, int flags) {
    ...
    TEST("lpNextRandom corner cases") {
        ...
        i = 0; TEST_ASSERT(lpNextRandom(lp, lpFirst(lp), &i, 0, 0) == NULL);

        /* Pick all returns all. */
        i = 0; TEST_ASSERT(lpNextRandom(lp, p0, &i, 3, 0) == p0 && i == 0);
        i = 1; TEST_ASSERT(lpNextRandom(lp, p1, &i, 2, 0) == p1 && i == 1);
        i = 2; TEST_ASSERT(lpNextRandom(lp, p2, &i, 1, 0) == p2 && i == 2);

        /* Pick more than one when there's only one left returns the last one. */
        i = 2; TEST_ASSERT(lpNextRandom(lp, p2, &i, 42, 0) == p2 && i == 2);

        /* Pick all even elements returns p0 and p2. */
        i = 0; TEST_ASSERT(lpNextRandom(lp, p0, &i, 10, 1) == p0 && i == 0);
        i = 1; TEST_ASSERT(lpNextRandom(lp, p1, &i, 10, 1) == p2 && i == 2);
