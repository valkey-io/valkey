...
Keep the number of INFO fields small. Before adding a metric, check whether users can
derive it from existing fields. Choose its section based on the intended audience
and whether the field is expected to remain stable:

- Metrics that are broadly useful to users belong in a default section, returned by
  `INFO` and `INFO default`. Document these fields in the INFO command documentation.
- Stable metrics for specific features or less common troubleshooting needs belong
  in a non-default section. Document these fields too. Non-default server sections
  are included in `INFO all`, which monitoring tools may collect, so avoid adding
  unnecessary fields just because a section is not returned by default.
- Internal debugging metrics that may change or be removed belong in the `debug`
  section and are not part of the user documentation. This section must be requested
  explicitly with `INFO debug`; it is excluded from `INFO`, `INFO all`, and
  `INFO everything`.

The default section list and section selection logic are in `genInfoSectionDict()`
and `genValkeyInfoString()` in `src/server.c`.
