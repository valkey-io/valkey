...
# Tests check gossip-specific log messages ("Sending ping packet", "reported node as not reachable").
...
start_cluster 4 0 {tags {external:skip cluster cluster-raft:skip} overrides {cluster-node-timeout 1000 loglevel "debug" cluster-announce-human-nodename "''"}} {
...
start_cluster 4 0 {tags {external:skip cluster ipv6 cluster-raft:skip} overrides {bind {127.0.0.1 ::1} cluster-announce-ip ::1 loglevel "debug" cluster-announce-human-nodename "''"}} {
    set RO_node_id [dict get [cluster_get_myself 0] id]
    set R0_port [srv 0 port]
    set R1_port [srv -1 port]
    set R2_port [srv -2 port]
