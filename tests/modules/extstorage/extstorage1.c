...
static int setFunction(ValkeyModuleCtx *module_ctx,
  ...
  ValkeyModuleExternalStorageState state =
      waitExternalStorageReady(storage_ctx);
  ValkeyModule_Assert(state == VMES_STATE_READONLY ||
...
static int delFunction(ValkeyModuleCtx *module_ctx,
  ...
  if (state == VMES_STATE_READONLY) {
    ValkeyModule_ReplyWithError(module_ctx,
                                "ERR External storage blocked readonly");
    return 0;
  }
...
int ValkeyModule_OnLoad(ValkeyModuleCtx *ctx, ValkeyModuleString **argv,
  ...
  return ValkeyModule_RegisterExternalStorage(ctx, storage_name, &methods);
}

int ValkeyModule_OnUnload(ValkeyModuleCtx *ctx) {
  int res = ValkeyModule_UnregisterExternalStorage(ctx, storage_name);
