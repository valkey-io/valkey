...
start_cluster 3 0 {tags {external:skip cluster} overrides {cluster-node-timeout 1000}} {
    test "cluster bus admin byte stats move on a healthy cluster" {
        wait_for_condition 1000 50 {
            [CI 0 cluster_stats_admin_bytes_sent] >= 1 &&
            [CI 0 cluster_stats_admin_bytes_received] >= 1
        } else {
            fail "R 0 admin byte stats are not as expected"
        }
    }
    ...
    test "CONFIG RESETSTAT resets cluster related stats" {
        ...
        assert_equal [getInfoProperty $info cluster_stats_admin_bytes_sent] 0
        assert_equal [getInfoProperty $info cluster_stats_admin_bytes_received] 0
