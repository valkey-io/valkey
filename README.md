...
The above runs the main integration tests. Additional tests are started using:
    ...
    % make test-unit     # Unit tests (both C unit tests and google unit tests)
...
- `-DBUILD_UNIT_GTESTS=[yes|no]`  when set, the build will produce google unit tests executable `valkey-unit-gtests`. Default: `no`
