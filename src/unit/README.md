...
If any `EXPECT` assertions are used in a file, the test must start with `TEST_BEGIN()` and end with `TEST_END()` to setup and return the status of `EXPECT` assertions.
