 ...
 */
 ...
 const char *storage_name = "hellostorage1";

 #define MAX_DB 16
 ValkeyModuleDict *mem_pool[MAX_DB];

static ValkeyModuleExternalStorageState waitExternalStorageReady(ValkeyModuleExternalStorageCtx *storage_ctx) {
...
static int setFunction(ValkeyModuleCtx *module_ctx,
    ...
    ValkeyModuleString *resulting_value = ValkeyModule_DictGet(mem_pool[dbid], (ValkeyModuleString *)key, NULL);
...
static int getFunction(ValkeyModuleCtx *module_ctx,
    ...
    void **found) {
    ValkeyModule_AutoMemory(module_ctx);
    ValkeyModule_Assert(module_ctx != NULL && storage_ctx != NULL && key_ctx != NULL);
    VALKEYMODULE_NOT_USED(storage_ctx);
    VALKEYMODULE_NOT_USED(key_ctx);
    ...
    void *value = ValkeyModule_DictGet(mem_pool[dbid], (ValkeyModuleString *)key, NULL);
    if (!value) {
        ValkeyModule_ReplyWithSimpleString(module_ctx, "OK");
        return 0;        
    }
...
static int delFunction(ValkeyModuleCtx *module_ctx,
    ...
    ValkeyModule_Assert(state == VMES_STATE_READONLY || state == VMES_STATE_READY);
    if (state == VMES_STATE_READONLY) {
        ValkeyModule_ReplyWithError(module_ctx, "ERR External storage blocked readonly");
        return 0;
    }
    ...
    ValkeyModuleString *value = ValkeyModule_DictGet(mem_pool[dbid], (ValkeyModuleString *)key, NULL);
    if (!value) {
        ValkeyModule_ReplyWithSimpleString(module_ctx, "OK");
        return 1;        
    }

    if (ValkeyModule_DictDel(mem_pool[dbid], (ValkeyModuleString *)key, NULL) != VALKEYMODULE_OK) {
        ValkeyModule_ReplyWithErrorFormat(module_ctx,
            "ERR Failed to del key %s",
            ValkeyModule_StringPtrLen(key, NULL));
        return 0;
    }
    *found = value;
...
int ValkeyModule_OnLoad(ValkeyModuleCtx *ctx,
    ...
    if (ValkeyModule_Init(ctx, storage_name, 1, VALKEYMODULE_APIVER_1) == VALKEYMODULE_ERR)
        return VALKEYMODULE_ERR;

    ValkeyModuleExternalStorageMethods methods = {
    ...
    return ValkeyModule_RegisterExternalStorage(
        ctx,
        storage_name,
        &methods);
}
...
int ValkeyModule_OnUnload(ValkeyModuleCtx *ctx) {
    return ValkeyModule_UnregisterExternalStorage(ctx, storage_name);
}
