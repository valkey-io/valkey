...
static int blockonkeys_kill_successor_timeout(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    REDISMODULE_NOT_USED(argv);
    REDISMODULE_NOT_USED(argc);
    return RedisModule_ReplyWithSimpleString(ctx, "TIMEOUT");
}

static void blockonkeys_kill_successor_free(RedisModuleCtx *ctx, void *privdata) {
    REDISMODULE_NOT_USED(ctx);
    RedisModule_Free(privdata);
}

static int blockonkeys_kill_successor_reply(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    REDISMODULE_NOT_USED(argv);
    REDISMODULE_NOT_USED(argc);
    int *role = RedisModule_GetBlockedClientPrivateData(ctx);
    if (role && *role == 1 && blockonkeys_kill_successor_id) {
        RedisModuleCallReply *r = RedisModule_Call(ctx, "CLIENT", "ccl", "KILL", "ID",
                                                   (long long)blockonkeys_kill_successor_id);
        if (r) RedisModule_FreeCallReply(r);
    }
    return RedisModule_ReplyWithSimpleString(ctx, role && *role == 1 ? "FIRST" : "SECOND");
}
...
static int blockonkeys_block_kill_successor(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    if (argc != 3) return RedisModule_WrongArity(ctx);

    size_t len = 0;
    const char *role_str = RedisModule_StringPtrLen(argv[2], &len);
    int *role = RedisModule_Alloc(sizeof(*role));
    if (len == 5 && !memcmp(role_str, "first", 5)) {
        *role = 1;
    } else if (len == 6 && !memcmp(role_str, "second", 6)) {
        *role = 2;
        blockonkeys_kill_successor_id = RedisModule_GetClientId(ctx);
    } else {
        RedisModule_Free(role);
        return RedisModule_ReplyWithError(ctx, "ERR role must be first|second");
    }

    RedisModule_BlockClientOnKeys(ctx, blockonkeys_kill_successor_reply,
...
static int blockonkeys_signal_ready(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    if (argc != 2) return RedisModule_WrongArity(ctx);
    RedisModuleCallReply *r = RedisModule_Call(ctx, "SET", "sc", argv[1], "x");
    if (r) RedisModule_FreeCallReply(r);
    RedisModule_SignalKeyAsReady(ctx, argv[1]);
    return RedisModule_ReplyWithSimpleString(ctx, "OK");
}
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (RedisModule_CreateCommand(ctx, "blockonkeys.block_kill_successor",
                                  blockonkeys_block_kill_successor, "", 1, 1, 1) == REDISMODULE_ERR)
        return REDISMODULE_ERR;

    if (RedisModule_CreateCommand(ctx, "blockonkeys.signal_ready",
                                  blockonkeys_signal_ready, "write", 1, 1, 1) == REDISMODULE_ERR)
        return REDISMODULE_ERR;
