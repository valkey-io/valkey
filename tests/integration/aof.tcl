...
tags {"aof external:skip logreqres:skip"} {
    ...
    create_aof $aof_dirpath $aof_file {
        ...
        append_to_aof [formatCommand set bar 2]
        append_to_aof [formatCommand exec]
        append_to_aof [formatCommand set baz 3]
    }
    ...
    start_server_aof [list dir $server_path aof-load-truncated yes] {
        test "AOF MULTI/EXEC transaction is loaded in full" {
            set client [valkey [srv host] [srv port] 0 $::tls]
            wait_done_loading $client
            assert_equal 1 [$client get foo]
            assert_equal 2 [$client get bar]
            assert_equal 3 [$client get baz]
        }
    }
