...
int test_metadata_read_write(int argc, char **argv, int flags) {
    ...
    /* Verify metadata size is configured for this object */
...
int test_metadata_multiple_objects(int argc, char **argv, int flags) {
    ...
    /* Verify metadata is configured for all objects */
    TEST_ASSERT(objectGetMetadataSize(obj_with_key1) == sizeof(objMetadata));
    TEST_ASSERT(objectGetMetadataSize(obj_with_key2) == sizeof(objMetadata));
    TEST_ASSERT(objectGetMetadataSize(obj_with_key3) == sizeof(objMetadata));
    ...
    TEST_EXPECT(objectGetMetaInt(obj_with_key1) == 100);
    TEST_EXPECT(objectGetMetaInt(obj_with_key2) == 200);
    TEST_EXPECT(objectGetMetaInt(obj_with_key3) == 300);

    /* Modify one and verify others are unchanged */
    objectSetMetaInt(obj_with_key2, 999);
    TEST_EXPECT(objectGetMetaInt(obj_with_key1) == 100);
    TEST_EXPECT(objectGetMetaInt(obj_with_key2) == 999);
    TEST_EXPECT(objectGetMetaInt(obj_with_key3) == 300);
