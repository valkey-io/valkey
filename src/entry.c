 ...
 *
 * Entry with string reference, used when the value is not owned by entry. The field is SDS
 * type 8 or higher.
 *
 *     +--------------+-----------+--------------+
 *     | Expiration   | string    | field        |
 *     | 1234567890LL | reference | hdr "foo" \0 |
 *     +--------------+-----^-----+--------------+
 *                          |
 *                          |
 *                          string reference = value is stringRef type.
 */
