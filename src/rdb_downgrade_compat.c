 ...
 *
 * This module provides compatibility functions to enable Redis 6.2
...
/* Listpack format constants for Redis 6.x compatibility */
#define LP_HDR_SIZE                                                            \
...
void rdbDowngradeStatsUpdateWithType(int success, size_t bytes, const char *key,
  ...
  if (success) {
    ...
    serverLog(
        LL_VERBOSE,
        "RDB downgrade success: key=%s, type=%s, bytes=%zu, total_keys=%llu",
        key ? key : "unknown", getDataStructureType(rdbtype), bytes,
        rdb_downgrade_stats.keys_converted);
  } else {
    rdb_downgrade_stats.keys_failed++;
    serverLog(LL_WARNING,
              "RDB downgrade failed: key=%s, type=%s, total_failures=%llu",
              key ? key : "unknown", getDataStructureType(rdbtype),
              rdb_downgrade_stats.keys_failed);
  }
...
void rdbDowngradeStatsUpdateWithTypeAndVersion(int success, size_t bytes,
                                               const char *key, int rdbtype,
                                               int rdbver) {
  /* Only increment statistics for RDB versions that actually require downgrade
   * conversion */
  if (rdbver < RDB_VERSION_REDIS_70) {
    serverLog(LL_DEBUG,
              "RDB compatibility: Skipping statistics update for RDB v%d "
              "key=%s (no conversion needed)",
              rdbver, key);
    return;
  }
...
sds rdbDowngradeStatsInfoString(void) {
  return sdscatprintf(
      ...
      rdb_downgrade_stats.keys_attempted, rdb_downgrade_stats.keys_succeeded,
      rdb_downgrade_stats.keys_failed, rdb_downgrade_stats.bytes_converted,
...
int isListpackEncoded(unsigned char *data, size_t len) {
  ...
  /* For RDB v11 compatibility, we need to be more aggressive in detecting
   * listpacks since Valkey consistently uses listpack format for RDB v11 */

  /* Basic listpack header check */
  if (len < 6)
    return 0; /* Need at least 6 bytes for header */
  ...
  if (total_bytes == len && total_bytes >= LISTPACK_MIN_VALID_SIZE &&
      total_bytes <= MAX_LISTPACK_SIZE && len > 0 && data[len - 1] == LP_EOF) {
  ...
  if (len >= 10) {
    ...
    if (zl_bytes == len && zl_tail_offset < len && len > 10 &&
        data[len - 1] == 0xFF) { /* ziplist EOF marker */
      return 0;
    }
  }

  /* For RDB v11, if it doesn't look like a ziplist and has basic listpack
   * structure, assume listpack */
...
unsigned char *listpackToZiplist(unsigned char *lp) {
  if (!lp) {
    serverLog(LL_WARNING,
              "RDB compatibility: listpackToZiplist called with NULL pointer");
    return NULL;
  }
  ...
  if (lp_size > MAX_LISTPACK_SIZE || lp_size < LISTPACK_MIN_VALID_SIZE) {
    serverLog(LL_WARNING,
              "listpackToZiplist: invalid listpack size (%zu bytes)", lp_size);
    return NULL;
  }
  ...
  if (num_elements > MAX_CONVERSION_ELEMENTS) {
    serverLog(LL_WARNING, "listpackToZiplist: too many elements (%u)",
              num_elements);
    return NULL;
  }
  ...
  while (p && processed < num_elements) {
    ...
    if (sval) {
      ...
      if (slen > (int64_t)server.proto_max_bulk_len) {
        serverLog(LL_WARNING,
                  "listpackToZiplist: string element too large (%lld bytes)",
                  (long long)slen);
        goto error_cleanup;
      }
    ...
    if (!new_zl) {
      serverLog(LL_WARNING, "listpackToZiplist: failed to add element %u",
                processed);
      goto error_cleanup;
    }
    ...
    if (!next_p && processed < num_elements) {
      serverLog(LL_WARNING,
                "listpackToZiplist: unexpected end of listpack at element %u",
                processed);
      goto error_cleanup;
    }
    ...
    if (processed >= MAX_CONVERSION_ELEMENTS) {
      serverLog(LL_WARNING,
                "listpackToZiplist: conversion element limit exceeded");
      goto error_cleanup;
    }
  }
  ...
  if (processed != num_elements) {
    serverLog(LL_WARNING,
              "listpackToZiplist: element count mismatch (expected %u, got %u)",
...
robj *rdbLoadObjectCompat(int rdbtype, rio *rdb, sds key, int *error,
  ...
  if (!rdb || !key) {
    serverLog(LL_WARNING,
              "RDB compatibility: Invalid arguments - rdb=%p, key=%s",
              (void *)rdb, key);
    if (error)
  ...
  if (rdbver < RDB_VERSION_REDIS_70) {
    serverLog(LL_DEBUG,
              "RDB compatibility: Using standard loader for RDB v%d key=%s",
              rdbver, key);
    return rdbLoadObject(rdbtype, rdb, key, error);
  }

  /* Handle RDB v11 stream type 21 (RDB_TYPE_STREAM_LISTPACKS_3) with
   * active_time support */
  if (rdbtype == RDB_TYPE_STREAM_LISTPACKS_3) {
    serverLog(LL_VERBOSE,
              "RDB compatibility: Processing RDB_TYPE_STREAM_LISTPACKS_3 (type "
              "21) for key=%s",
              key);

    /* Load stream as RDB_TYPE_STREAM_LISTPACKS but handle extra active_time
     * fields */
  ...
  serverLog(LL_VERBOSE, "RDB compatibility: Processing RDB v%d key=%s type=%s",
            rdbver, key, getDataStructureType(rdbtype));
  ...
  if (requiresListpackConversion(rdbtype, rdbver) != 0) {
    ...
    if (rdbtype == RDB_TYPE_LIST_QUICKLIST_2) {
      serverLog(LL_VERBOSE,
                "RDB compatibility: Processing LIST_QUICKLIST_2 for key=%s, "
                "type=%s (RDB v%d)",
      ...
      if ((len = rdbLoadLen(rdb, NULL)) == RDB_LENERR) {
        serverLog(
            LL_WARNING,
            "RDB compatibility: Failed to load quicklist length for key=%s",
            key);
        goto error_cleanup;
      }
      if (len == 0) {
      ...
      if (!o) {
        serverLog(
            LL_WARNING,
            "RDB compatibility: Failed to create quicklist object for key=%s",
            key);
        goto error_cleanup;
      }
      ...
      while (len--) {
        ...
        if ((container = rdbLoadLen(rdb, NULL)) == RDB_LENERR) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Failed to load quicklist container "
                    "type for key=%s",
                    key);
          decrRefCount(o);
          goto error_cleanup;
        ...
        if (container != QUICKLIST_NODE_CONTAINER_PACKED &&
          ...
          serverLog(LL_WARNING,
                    "RDB compatibility: Invalid quicklist container type %llu "
                    "for key=%s",
                    (unsigned long long)container, key);
          decrRefCount(o);
          goto error_cleanup;
        }
        ...
        if (!node_encoded) {
          serverLog(
              LL_WARNING,
              "RDB compatibility: Failed to load quicklist node for key=%s",
              key);
          decrRefCount(o);
          goto error_cleanup;
        }
        ...
        if (container == QUICKLIST_NODE_CONTAINER_PLAIN) {
          ...
          zl = ziplistNew();
          if (!quicklistConvertAndValidateIntegrity(node_encoded,
                                                    node_encoded_len, &zl)) {
            serverLog(LL_WARNING,
                      "RDB compatibility: listpack to ziplist conversion "
                      "failed for quicklist node key=%s, skipping node",
                      key);
            zfree(node_encoded);
            zfree(zl);
      ...
      if (quicklistCount(o->ptr) == 0) {
        serverLog(LL_WARNING,
                  "RDB compatibility: Created empty quicklist for key=%s", key);
      }

      if (conversion_attempted) {
        rdbDowngradeStatsUpdateWithTypeAndVersion(1, total_converted_size, key,
                                                  rdbtype, rdbver);
        serverLog(LL_VERBOSE,
                  "RDB compatibility: Successfully converted listpack to "
                  "ziplist for key=%s, type=%s (RDB v%d, %zu->%zu bytes)",
                  key, getDataStructureType(rdbtype), rdbver,
                  total_original_size, total_converted_size);
      }

      mstime_t elapsed = mstime() - start_time;
      serverLog(LL_VERBOSE,
                "RDB compatibility: Successfully processed LIST_QUICKLIST_2 "
                "key=%s, type=%s in %lldms",
                key, getDataStructureType(rdbtype), elapsed);
      ...
      conversion_attempted = 1;
      serverLog(LL_DEBUG,
                "RDB compatibility: Attempting conversion for key=%s, type=%s",
                key, getDataStructureType(rdbtype));

      /* Load the raw data with comprehensive error handling */
      encoded = rdbGenericLoadStringObject(rdb, RDB_LOAD_PLAIN, &encoded_len);
      if (!encoded) {
        serverLog(LL_WARNING,
                  "RDB compatibility: Failed to load raw data for key=%s", key);
        goto error_cleanup;
      }

      original_size = encoded_len;
      serverLog(LL_DEBUG,
                "RDB compatibility: Loaded raw data size=%zu for key=%s",
                encoded_len, key);
      ...
      if (encoded_len > MAX_LISTPACK_SIZE) {
        serverLog(
            LL_WARNING,
            "RDB compatibility: Data too large (%zu bytes) for key=%s, max=%d",
      ...
      if (isListpackEncoded(encoded, encoded_len)) {
        serverLog(LL_VERBOSE,
                  "RDB compatibility: Detected listpack encoding for key=%s, "
                  "type=%s, converting...",
                  key, getDataStructureType(rdbtype));
        ...
        if (!zl) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Listpack to ziplist conversion failed "
                    "for key=%s, type=%s",
                    key, getDataStructureType(rdbtype));
          goto error_cleanup;
        }
        ...
        if (zl_len == 0 || zl_len > MAX_LISTPACK_SIZE) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Invalid ziplist size (%zu) for key=%s",
                    zl_len, key);
          zfree(zl);
          goto error_cleanup;
        }
        ...
        serverLog(LL_VERBOSE,
                  "RDB compatibility: Successfully converted listpack to "
                  "ziplist for key=%s, type=%s (RDB v%d, %zu->%zu bytes)",
                  key, getDataStructureType(rdbtype), rdbver, original_size,
                  converted_size);
      } else {
        /* Data is already in ziplist format */
        serverLog(LL_DEBUG,
                  "RDB compatibility: Data already in ziplist format for "
                  "key=%s, type=%s",
                  key, getDataStructureType(rdbtype));
      ...
      if (!encoded || encoded_len == 0) {
        serverLog(LL_WARNING,
                  "RDB compatibility: No data to create object for key=%s",
                  key);
        goto error_cleanup;
      }
      ...
      if (!o) {
        serverLog(LL_WARNING,
                  "RDB compatibility: Object creation failed for key=%s", key);
        goto error_cleanup;
      }
      ...
      case RDB_TYPE_HASH_LISTPACK:
        ...
        if (!ziplistValidateIntegrity(o->ptr, converted_size, 1, NULL, NULL)) {
          serverLog(
              LL_WARNING,
              "RDB compatibility: Invalid hash ziplist structure for key=%s",
              key);
          decrRefCount(o);
          goto error_cleanup;
        }
        ...
        if (hashTypeLength(o) > server.hash_max_ziplist_entries) {
          serverLog(LL_DEBUG,
                    "RDB compatibility: Converting hash to HT encoding for "
                    "key=%s (size=%lu)",
      ...
      case RDB_TYPE_ZSET_LISTPACK:
        ...
        if (!zsetZiplistValidateIntegrity(o->ptr, converted_size, 1)) {
          serverLog(
              LL_WARNING,
              "RDB compatibility: Invalid zset ziplist structure for key=%s",
              key);
          decrRefCount(o);
          goto error_cleanup;
        }
        ...
        if (zsetLength(o) > server.zset_max_ziplist_entries) {
          serverLog(LL_DEBUG,
                    "RDB compatibility: Converting zset to skiplist encoding "
                    "for key=%s (size=%lu)",
                    key, zsetLength(o));
      ...
      case RDB_TYPE_LIST_ZIPLIST:
        ...
        if (!ziplistValidateIntegrity(o->ptr, converted_size, 1, NULL, NULL)) {
          serverLog(
              LL_WARNING,
              "RDB compatibility: Invalid list ziplist structure for key=%s",
              key);
          decrRefCount(o);
          goto error_cleanup;
        }

        /* Convert to quicklist as Redis 6.2 expects */
        serverLog(LL_DEBUG,
                  "RDB compatibility: Converting list to quicklist encoding "
                  "for key=%s",
                  key);
      ...
      case RDB_TYPE_SET_LISTPACK:
        ...
        if (!set_dict) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Failed to create set dict for key=%s",
                    key);
          decrRefCount(o);
          goto error_cleanup;
        }
        ...
        while (p) {
          ...
          if (ziplistGet(p, &vstr, &vlen, &vlong)) {
            ...
            if (dictAdd(set_dict, element, NULL) != DICT_OK) {
              serverLog(LL_WARNING,
                        "RDB compatibility: Duplicate set element for key=%s",
                        key);
              sdsfree(element);
      ...
      default:
        serverLog(LL_WARNING,
                  "RDB compatibility: Unexpected RDB type %s for key=%s",
                  getDataStructureType(rdbtype), key);
        decrRefCount(o);
        goto error_cleanup;
      }
      ...
      if (conversion_attempted) {
        rdbDowngradeStatsUpdateWithTypeAndVersion(1, converted_size, key,
                                                  rdbtype, rdbver);
      }
      ...
      serverLog(
          LL_VERBOSE,
          "RDB compatibility: Successfully processed key=%s, type=%s in %lldms",
          key, getDataStructureType(rdbtype), elapsed);
  ...
  serverLog(LL_DEBUG,
            "RDB compatibility: Using standard loader fallback for key=%s, "
            "type=%s rdb_v%d",
            key, getDataStructureType(rdbtype), rdbver);

  mstime_t fallback_elapsed = mstime() - start_time;
  serverLog(LL_DEBUG,
            "RDB compatibility: Fallback completed for key=%s in %lldms", key,
            fallback_elapsed);
...
error_cleanup:
  ...
  if (conversion_attempted) {
    rdbDowngradeStatsUpdateWithTypeAndVersion(0, original_size, key, rdbtype,
                                              rdbver);
  }

  mstime_t error_elapsed = mstime() - start_time;
  serverLog(LL_WARNING,
            "RDB compatibility: Failed to process key=%s, type=%s after %lldms",
            key, getDataStructureType(rdbtype), error_elapsed);
...
robj *rdbLoadStreamWithActiveTime(rio *rdb, sds key, int *error, int rdbver) {
  ...
  serverLog(LL_VERBOSE,
            "RDB compatibility: Loading stream with active_time support for "
            "key=%s (RDB v%d)",
            key, rdbver);
  ...
  if (!o) {
    serverLog(LL_WARNING,
              "RDB compatibility: Failed to create stream object for key=%s",
              key);
    return NULL;
  }
  s = o->ptr;
  ...
  if (listpacks == RDB_LENERR) {
    serverLog(
        LL_WARNING,
        "RDB compatibility: Failed to load listpacks count for stream key=%s",
        key);
    decrRefCount(o);
    return NULL;
  }
  ...
  while (listpacks--) {
    ...
    if (!nodekey) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to load stream master ID for key=%s",
                key);
      decrRefCount(o);
      return NULL;
    }
    ...
    if (sdslen(nodekey) != sizeof(streamID)) {
      serverLog(LL_WARNING,
                "RDB compatibility: Invalid stream node key size for key=%s",
                key);
      sdsfree(nodekey);
      decrRefCount(o);
      return NULL;
    ...
    if (!lp) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to load stream listpack for key=%s",
                key);
      sdsfree(nodekey);
      decrRefCount(o);
      return NULL;
    ...
    /* Validate listpack */
    if (!streamValidateListpackIntegrity(lp, lp_size, 1)) {
      serverLog(LL_WARNING,
                "RDB compatibility: Stream listpack integrity check failed for "
                "key=%s",
                key);
      sdsfree(nodekey);
      zfree(lp);
    ...
    if (!first) {
      serverLog(LL_WARNING,
                "RDB compatibility: Empty listpack in stream for key=%s", key);
      sdsfree(nodekey);
      zfree(lp);
      decrRefCount(o);
      return NULL;
    ...
    if (!raxTryInsert(s->rax, (unsigned char *)nodekey, sizeof(streamID), lp,
                      NULL)) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to insert listpack into stream "
                "radix tree for key=%s",
                key);
      sdsfree(nodekey);
      zfree(lp);
      decrRefCount(o);
  ...
  if (rioGetReadError(rdb)) {
    serverLog(LL_WARNING,
              "RDB compatibility: Failed to load stream metadata for key=%s",
              key);
    decrRefCount(o);
    return NULL;
  }
  ...
  if (cgroups_count == RDB_LENERR) {
    serverLog(LL_WARNING,
              "RDB compatibility: Failed to load consumer groups count for "
              "stream key=%s",
              key);
    decrRefCount(o);
    return NULL;
  ...
  while (cgroups_count--) {
    ...
    if (!cgname) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to load consumer group name for "
                "stream key=%s",
                key);
      decrRefCount(o);
      return NULL;
    ...
    if (rioGetReadError(rdb)) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to load consumer group ID for "
                "stream key=%s",
                key);
      sdsfree(cgname);
      decrRefCount(o);
    ...
    if (!cgroup) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to create consumer group for stream "
                "key=%s",
                key);
      sdsfree(cgname);
      decrRefCount(o);
    ...
    if (pel_size == RDB_LENERR) {
      serverLog(LL_WARNING,
                "RDB compatibility: Failed to load PEL size for stream key=%s",
                key);
      decrRefCount(o);
      return NULL;
    }
    ...
    while (pel_size--) {
      ...
      if (rioRead(rdb, rawid, sizeof(rawid)) == 0) {
        serverLog(LL_WARNING,
                  "RDB compatibility: Failed to load PEL ID for stream key=%s",
                  key);
        decrRefCount(o);
        return NULL;
      }
      ...
      if (rioGetReadError(rdb)) {
        serverLog(
            LL_WARNING,
            "RDB compatibility: Failed to load NACK data for stream key=%s",
            key);
        decrRefCount(o);
        streamFreeNACK(nack);
        return NULL;
      ...
      if (!raxTryInsert(cgroup->pel, rawid, sizeof(rawid), nack, NULL)) {
        serverLog(
            LL_WARNING,
            "RDB compatibility: Failed to insert PEL entry for stream key=%s",
            key);
        decrRefCount(o);
        streamFreeNACK(nack);
        return NULL;
    ...
    if (consumers_num == RDB_LENERR) {
      serverLog(
          LL_WARNING,
          "RDB compatibility: Failed to load consumers count for stream key=%s",
          key);
      decrRefCount(o);
      return NULL;
    }
    ...
    while (consumers_num--) {
      ...
      if (!cname) {
        serverLog(
            LL_WARNING,
            "RDB compatibility: Failed to load consumer name for stream key=%s",
            key);
        decrRefCount(o);
        return NULL;
      }
      ...
      if (rioGetReadError(rdb)) {
        serverLog(LL_WARNING,
                  "RDB compatibility: Failed to load consumer seen_time for "
                  "stream key=%s",
                  key);
        decrRefCount(o);
        return NULL;
      ...
      if (rdbver >= RDB_VERSION_VALKEY_80) {
        /* RDB v11 has active_time field - read and discard it, set active_time
         * = seen_time */
        long long active_time = rdbLoadMillisecondTime(rdb, RDB_VERSION);
        if (rioGetReadError(rdb)) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Failed to load consumer active_time "
                    "for stream key=%s",
                    key);
          decrRefCount(o);
          return NULL;
        ...
        /* For Redis 6.2 compatibility, we don't have active_time field, so we
         * ignore it */
        /* The consumer struct in Redis 6.2 only has seen_time */
        serverLog(LL_DEBUG,
                  "RDB compatibility: Read and discarded active_time=%lld for "
                  "consumer in stream key=%s",
                  active_time, key);
      }
      ...
      if (consumer_pel_size == RDB_LENERR) {
        serverLog(LL_WARNING,
                  "RDB compatibility: Failed to load consumer PEL size for "
                  "stream key=%s",
                  key);
        decrRefCount(o);
        return NULL;
      ...
      while (consumer_pel_size--) {
        ...
        if (rioRead(rdb, rawid, sizeof(rawid)) == 0) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Failed to load consumer PEL ID for "
                    "stream key=%s",
                    key);
          decrRefCount(o);
          return NULL;
        ...
        if (nack == raxNotFound) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Consumer PEL entry not found in global "
                    "PEL for stream key=%s",
                    key);
          decrRefCount(o);
          return NULL;
        }
        ...
        if (!raxTryInsert(consumer->pel, rawid, sizeof(rawid), nack, NULL)) {
          serverLog(LL_WARNING,
                    "RDB compatibility: Failed to insert consumer PEL entry "
                    "for stream key=%s",
                    key);
          decrRefCount(o);
          return NULL;
  ...
  serverLog(LL_NOTICE,
            "RDB compatibility: Successfully loaded stream with active_time "
            "compatibility for key=%s",
            key);
 ...
 * The listpack element pointed by 'p' will be converted and stored into
 * ziplist. */
static int _listpackEntryConvertAndValidate(unsigned char *p,
...
int quicklistConvertAndValidateIntegrity(unsigned char *lp, size_t size,
                                         unsigned char **zl) {
  if (!lpValidateIntegrity(lp, size, 1))
    return 0;
  unsigned char *p = lpFirst(lp);
  while (p) {
