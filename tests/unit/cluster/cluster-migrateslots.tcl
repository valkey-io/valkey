...
# These three wait on work that completes asynchronously across the cluster, so
# they are bounded by the same 50s the rest of the cluster suite allows for
...
proc wait_for_migration {node_idx slot} {
    ...
    wait_for_condition 500 100 {
        ...
        set nodes [get_cluster_nodes $node_idx]
        fail "Cluster node $target_id did not get slot $slot within 50000 ms (current $nodes)"
    }
