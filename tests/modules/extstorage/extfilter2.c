...
static ValkeyModuleExternalFilterState waitExternalFilterReady(ValkeyModuleExternalFilterCtx *filter_ctx) {
...
static int getFunction(ValkeyModuleCtx *module_ctx,
    ...
    void **found) {
    ValkeyModule_AutoMemory(module_ctx);
    ValkeyModule_Assert(module_ctx != NULL && filter_ctx != NULL && key_ctx != NULL);
    VALKEYMODULE_NOT_USED(filter_ctx);
    VALKEYMODULE_NOT_USED(key_ctx);
...
static int delFunction(ValkeyModuleCtx *module_ctx,
    ...
    ValkeyModule_Assert(state == VMEF_STATE_READONLY || state == VMEF_STATE_READY);
    if (state == VMEF_STATE_READONLY) {
        ValkeyModule_ReplyWithError(module_ctx, "ERR External filter blocked readonly");
        return 0;
    }
    ...
    *found = ValkeyModule_CreateStringFromLongLong(module_ctx, 1);
...
int ValkeyModule_OnLoad(ValkeyModuleCtx *ctx,
    ...
    if (ValkeyModule_Init(ctx, filter_name, 1, VALKEYMODULE_APIVER_1) == VALKEYMODULE_ERR)
    ...
    return ValkeyModule_RegisterExternalFilter(
        ctx,
        filter_name,
        &methods);
}
...
int ValkeyModule_OnUnload(ValkeyModuleCtx *ctx) {
    return ValkeyModule_UnregisterExternalFilter(ctx, filter_name);
}
