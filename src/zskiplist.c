...
/* Sentinel values for -inf/+inf in lexicographic range comparisons.
 * These only need unique addresses for pointer comparison. */
static char _zslMinString[] = "minstring";
static char _zslMaxString[] = "maxstring";
sds zslMinString = (sds)_zslMinString;
sds zslMaxString = (sds)_zslMaxString;

/*-----------------------------------------------------------------------------
 * Skiplist internal helpers
 *----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
 * Skiplist lex range queries
 *----------------------------------------------------------------------------*/

/* This is just a wrapper to sdscmp() that is able to
 * handle zslMinString and zslMaxString as the equivalent of
 * -inf and +inf for strings */
int sdscmplex(sds a, sds b) {
    if (a == b) return 0;
    if (a == zslMinString || b == zslMaxString) return -1;
    if (a == zslMaxString || b == zslMinString) return 1;
    return sdscmp(a, b);
}
