#!/usr/bin/env python3
"""Parse, render, and promote the 00-RELEASENOTES "## Unreleased" block.

The unstable branch keeps a "## Unreleased" block in 00-RELEASENOTES that
user-facing PRs append to as they merge. When a release is cut,
``promote()`` lifts that block into a dated, release-branch-style section and
resets the "## Unreleased" categories to empty so the next cycle starts clean.

This module is pure (no I/O, no network) so it is cheap to unit test. The CLI
entry points that read/write files live in prepare_release.py and
