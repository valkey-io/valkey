...
int test_scan_with_rehashed_long_chain(int argc, char **argv, int flags) {
    ...
    UNUSED(argv);
    UNUSED(flags);
    // creates hashset with elements displaced due to open addressing, such that on a rehash they will move "backwards"
    ...
    // add background random elements (helps to keep fill between limits, prevent unwanted rehash)
    ...
    // delete base of chain. This means that elements remaining on the end of the chain should move "backwards" on a rehash.
    for (size_t i = 0; i < num_chain_base_elements; i++) {
    ...
    // create a "train" of scans such that there is a scan paused at every cursor position where hashsetScan() pauses
    // rehash in step with 
