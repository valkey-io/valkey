...
all_versions = set()
all_commands = dict()
all_titles = ["name", "group", "since", "complexity"]
all_groups = [
    "string",
    "list",
    "set",
...
for command in command_list:
    cmd = dict()
    cmd["name"] = command.fullname()
    cmd["group"] = command.group
    cmd["since"] = get_optional_desc_string(command.desc, "since")
    cmd["complexity"] = get_optional_desc_string(command.desc, "complexity")

    if "history" in command.desc:
        for tupl in command.desc["history"]:
            version = tupl[0]
            change = tupl[1]
            all_versions.add(version)
            cmd[version] = change

    all_commands[command.fullname()] = cmd

    if command.subcommands:
        subcommand_list = sorted(command.subcommands, key=lambda cmd: cmd.desc["since"])
...
all_versions.sort()
all_titles.extend(all_versions)
df = pd.DataFrame(all_commands.values(), columns=all_titles)
df["since"] = df["since"].str.strip('"')
df["complexity"] = df["complexity"].str.strip('"')
df["group_order"] = df["group"].map({v:i for i,v in enumerate(all_groups)})
sorted_df = df.sort_values(
    by=["group_order", "since"],
    key=lambda x: x if x.name != "since" else x.str.split('.').apply(lambda v: tuple(map(int, v))),
    ascending=[True, True]
).drop("group_order", axis=1)
sorted_df.to_excel("commands.xlsx", index=False, engine="openpyxl")
