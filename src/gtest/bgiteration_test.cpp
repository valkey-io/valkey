...
// The private data is a pointer to arbitraty data.  This value is used just to
...
class BgIterationTest : public ::testing::Test {
    ...
    protected:
        ...
        void simulateActualWrite(client *c) {
            ...
            dbFind(c->db, static_cast<sds>(objectGetVal(c->argv[1])));
            ...
            dbEntry *de = dbFind(c->db, static_cast<sds>(objectGetVal(c->argv[1])));
            // JHB get the metadata and check the epoch == bgIteration_epoch
            UNUSED(de);
...
TEST_F(BgIterationTest, modFutureItem_NoReplication_YesConsistent_CloneExpeditedItem) {
    ...
    // We only call resume rehashing if we paused sucesfully on a hash
...
/////////////////////////////////////////////////////
// Simple Slots iterator tests
/////////////////////////////////////////////////////

#ifdef CODE_NOT_READY_YET
class BgIterationTestClusterSlots : public BgIterationTestCluster {
    protected:
        static constexpr int N_SLOTS = 4;
        static constexpr int N_KEYS_PER_SLOT = 5;
        static constexpr int N_KEYS = N_SLOTS * N_KEYS_PER_SLOT;

    private:
        // This is the expected order of the keys when hashed into a dict of size 8.
        // We have 1 DB and 4 slots (0 through 3).
        const char *keys[1][N_SLOTS][N_KEYS_PER_SLOT] = {{
            {"{06S}C0", "{06S}D0", "{06S}A0", "{06S}B0", "{06S}E0"}, // slot 0
            {"{emj}B0", "{emj}D0", "{emj}A0", "{emj}C0", "{emj}E0"}, // slot 1
            {"{yMm}A0", "{yMm}C0", "{yMm}E0", "{yMm}B0", "{yMm}D0"}, // slot 2
            {"{zho}A0", "{zho}B0", "{zho}D0", "{zho}C0", "{zho}E0"}, // slot 3
        }};

        const char *slot_hashtags[N_SLOTS] = {"{06S}", "{emj}", "{yMm}", "{zho}"};
    ...
    protected:
        const int slots_to_iterate[3] = {0, 1, 2};
        const int slots_to_iterate_size = 3;
        const int n_keys_to_read = slots_to_iterate_size * N_KEYS_PER_SLOT;

        virtual const char * getKeyAtDbSeq(int db, int seq) override {
            assert(db == 0);
            assert(seq < N_KEYS);
            return keys[db][seq / N_KEYS_PER_SLOT][seq % N_KEYS_PER_SLOT];
        }

        virtual int getDbFromItemNum(int itemNum) override {
            UNUSED(itemNum);
            return 0;
        }

        virtual int getSeqFromItemNum(int itemNum) override {
            return itemNum;
        }

        void addKeyToDb(int dbid, int slot, const char *key, const char *val) {
            sds sds_key = sdscat(sdsnew(slot_hashtags[slot]), key);
            sds sds_val = sdscat(sdsnew(slot_hashtags[slot]), val);
            addKeyAndValObjsToDb(dbid, sds_key, sds_val);
        }
        ...
        virtual void setupDatabase() override {
            ...
            for (int slot = 0;  slot < N_SLOTS;  slot++) {
                addKeyToDb(0, slot, "A0", "A0");
                addKeyToDb(0, slot, "B0", "B0");
                addKeyToDb(0, slot, "C0", "C0");
                addKeyToDb(0, slot, "D0", "D0");
                addKeyToDb(0, slot, "E0", "E0");
            }

            // In case we need to debug...
            if (0) debugPrintBucketInfo(N_SLOTS);

            // Validate that the iteration order matches the expected order
            kvstoreIterator *it = kvstoreIteratorInit(server.db[0]->keys, HASHTABLE_ITER_SAFE);
            for (int i = 0;  i < N_KEYS;  i++) {
                dbEntry *de;
                kvstoreIteratorNext(it, reinterpret_cast<void **>(&de));
                ASSERT_STREQ(static_cast<const char*>(objectGetKey(de)), getKeyAtDbSeq(0, i));
            }
            amzKvstoreIteratorRelease(it, server.db[0]->keys);
        }
}; // class BgIterationTestClusterSlots

TEST_F(BgIterationTestClusterSlots, dictIsOK) {
    // Just run the setup/teardown code to make sure the dict is OK.
}

TEST_F(BgIterationDeathTest, createSlotsIterStandalone_fail) {
    // A slot iterator is invalid in cluster mode disabled.
    const int slots[1] = {0};
    EXPECT_DEATH(bgIteratorCreateSlotsIter("iter", 0, slots, 1, NULL, NULL, NULL, NULL), "");
}

TEST_F(BgIterationTestClusterSlots, createAndCleanup) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple",
            0, slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    EXPECT_EQ(bgIteratorFind("simple"), it);
    EXPECT_STREQ(bgIteratorName(it), "simple");
...
TEST_F(BgIterationTestClusterSlots, orderedIteration3Slots) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple",
            0, slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    expectReadKeySequence(it, 0, n_keys_to_read - 1);

    // Quick status check.  At this point, the last item hasn't been returned yet.
    bgIteratorStatus status;
    bgIteratorGetStatus(it, &status);
    EXPECT_EQ(status.dbentries_queued, n_keys_to_read);
    EXPECT_EQ(status.dbentries_processed, n_keys_to_read - 1);
...
TEST_F(BgIterationTestClusterSlots, orderedIteration4SlotsUnordered) {
    const int slots_to_iterate_size = N_SLOTS;
    const int slots_to_iterate[slots_to_iterate_size] = {3, 1, 2, 0};
    const int n_keys_to_read = N_KEYS;
...
TEST_F(BgIterationTestClusterSlots, orderedIterationWithEmptySlots) {
    const int slots_to_iterate_size = 4;
    const int slots_to_iterate[slots_to_iterate_size] = {1, 2, 10, 11};
    const int n_keys_to_read = 2 * N_KEYS_PER_SLOT;

    bgIterator *it = bgIteratorCreateSlotsIter("simple",
            0, slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    expectReadKeySequence(it, N_KEYS_PER_SLOT, N_KEYS_PER_SLOT + n_keys_to_read - 1);
...
TEST_F(BgIterationTestClusterSlots, orderedIteration2SlotsWithGap) {
    const int slots_to_iterate_size = 2;
    const int slots_to_iterate[slots_to_iterate_size] = {1, 3};

    bgIterator *it = bgIteratorCreateSlotsIter("simple",
            0, slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    expectReadKeySequence(it, N_KEYS_PER_SLOT, N_KEYS_PER_SLOT * 2 - 1);

    unsigned expected_entries_queued = N_KEYS_PER_SLOT;
    bgIteratorStatus status;
    bgIteratorGetStatus(it, &status);
    EXPECT_GE(status.dbentries_queued, expected_entries_queued);
    EXPECT_EQ(status.dbentries_processed, expected_entries_queued - 1);

    expectReadKeySequence(it, N_KEYS_PER_SLOT * 3, N_KEYS_PER_SLOT * 4 - 1);

    expected_entries_queued = N_KEYS_PER_SLOT * 2;
    bgIteratorGetStatus(it, &status);
    EXPECT_EQ(status.dbentries_queued, expected_entries_queued);
    EXPECT_EQ(status.dbentries_processed, expected_entries_queued - 1);
...
TEST_F(BgIterationTestClusterSlots, iterationOnlyEmptySlots) {
    const int slots_to_iterate_size = 5;
    const int slots_to_iterate[slots_to_iterate_size] = {10, 11, 1000, 2000, 3000};
...
TEST_F(BgIterationTestClusterSlots, modifyFutureItem_FutureSlot) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    // Read slot 0
    expectReadKeySequence(it, 0, N_KEYS_PER_SLOT - 1);

    // Read the first key of slot 1
    expectReadKey(it, N_KEYS_PER_SLOT, nullptr);

    // We fake a modification to a key in slot 2 (in the future)
    const int modified_key = N_KEYS_PER_SLOT * 2 + 1;
    client *c = getWriteClient(modified_key, "xxx");
    ...
    expectReadKeySequence(it, N_KEYS_PER_SLOT + 1, modified_key - 1);

    // Let's validate that the modified key shows the new value
    expectReadKey(it, modified_key, "xxx");

    // Continue...
    expectReadKeySequence(it, modified_key + 1, n_keys_to_read - 1);
    expectReadComplete(it);
    freeTestClient(c);
}

TEST_F(BgIterationTestClusterSlots, modifyFutureItem_SameSlot) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    ...
    // We fake a modification to the last key in slot 1 (in the future)
    const int modified_key = N_KEYS_PER_SLOT * 2 - 1;
    client *c = getWriteClient(modified_key, "xxx");
...
TEST_F(BgIterationTestClusterSlots, modifyFutureItem_SameSlot_IterNotInit) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    // We fake a modification to the first key in slot 0,
    // which is in the future since we have not started the iteration yet.
    const int modified_key = 0;
    client *c = getWriteClient(modified_key, "xxx");
...
TEST_F(BgIterationTestClusterSlots, modifyOutOfScopeItem_GreaterSlot) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    ...
    // We fake a modification to a key in slot 3 (outside the scope of this iterator)
    client *c = getWriteClient(N_KEYS_PER_SLOT * 3 + 1, "xxx");

    // We DONT expect the client to be blocked
    simulateActualWrite(c);

    // NOTE:  Since since this key is outside of the scope of this iterator, we don't expect this
    //        command to be replicated.

    // Now continue reading
    expectReadKeySequence(it, N_KEYS_PER_SLOT, n_keys_to_read - 1);
    expectReadComplete(it);
    freeTestClient(c);
}

TEST_F(BgIterationTestClusterSlots, modifyOutOfScopeItem_LesserSlot) {
    const int slots_to_iterate_size = 1;
    const int slots_to_iterate[slots_to_iterate_size] = {1};

    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    // Read first key of slot 1
    expectReadKey(it, N_KEYS_PER_SLOT, nullptr);

    // We fake a modification to a key in slot 0 (outside the scope of this iterator)
    client *c = getWriteClient(0, "xxx");
    ...
    // Now continue reading
    expectReadKeySequence(it, N_KEYS_PER_SLOT + 1, N_KEYS_PER_SLOT * 2 - 1);
    expectReadComplete(it);
    freeTestClient(c);
}

TEST_F(BgIterationTestClusterSlots, modifyOutOfScopeItem_Consistent) {
    const int slots_to_iterate_size = 1;
    const int slots_to_iterate[slots_to_iterate_size] = {1};

    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_CONSISTENT,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
...
TEST_F(BgIterationTestClusterSlots, modifyPastItem_PastSlot) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    ...
    // Read the first key of slot 1 and enqueing the next item
    expectReadKey(it, N_KEYS_PER_SLOT, nullptr);

    // We fake a modification to a key in slot 0 (in the past)
    client *c = getWriteClient(1, "xxx");

    // We DONT expect the client to be blocked - not consistent
    simulateActualWrite(c);

    // Second key of slot 1 was already in queue. The replication will follow.
    expectReadKey(it, N_KEYS_PER_SLOT + 1, nullptr);
    expectReadReplication(it, c);

    // Continue...
    expectReadKeySequence(it, N_KEYS_PER_SLOT + 2, n_keys_to_read - 1);
    expectReadComplete(it);
    freeTestClient(c);
}

TEST_F(BgIterationTestClusterSlots, modifyPastItem_SameSlot) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    ...
    // Read the first two keys of slot 1 and enqueing the rest
    expectReadKeySequence(it, N_KEYS_PER_SLOT, N_KEYS_PER_SLOT + 1);

    // We fake a modification to the first key in slot 1 (in the past)
    client *c = getWriteClient(N_KEYS_PER_SLOT, "xxx");

    // We DONT expect the client to be blocked - not consistent
    simulateActualWrite(c);

    // The rest of the keys of slot 1 was already in queue. The
    // replication will follow.
    int end_of_slot_2 = 2 * N_KEYS_PER_SLOT - 1;
    expectReadKeySequence(it, N_KEYS_PER_SLOT + 2, end_of_slot_2);
    expectReadReplication(it, c);

    // Continue...
    expectReadKeySequence(it, end_of_slot_2 + 1, n_keys_to_read - 1);
    expectReadComplete(it);
    freeTestClient(c);
}

TEST_F(BgIterationTestClusterSlots, flushDB_flushAll) {
    bgIterator *it = bgIteratorCreateSlotsIter("simple",
            0, slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);

    int stop_key = n_keys_to_read / 2;
    expectReadKeySequence(it, 0, stop_key);

    // Only few keys should be in the queue, and they will be
    // returned to Valkey before the flush.
    simulateFlushDB(-1, stop_key);
...
TEST_F(BgIterationTestClusterSlots, modifyItems_MultiClient) {
    // This test is meant to replicate this bug: https://sim.amazon.com/issues/ELMO-109357
    bgIterator *it = bgIteratorCreateSlotsIter("simple", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    ...
    // Fake a modification to a key in slot 0 (should replicate).
    client *in_scope_client = getMultiClient("SET {06S}C0 xxx", 1);
    in_scope_client->db = server.db[0];

    // Simulate MULTI/EXEC command
    simulateUnblockedMultiExec(in_scope_client);
    
    // Read the previously queued key and feed the replication item
    expectReadKey(it, N_KEYS_PER_SLOT + 1, nullptr);

    // Ensure MULTI/EXEC block is replicated
    expectReadMultiReplication(it);
    expectReadReplication(it, in_scope_client);
    expectReadExecReplication(it);

    // Fake a modification to a key in slot 3 (out of key scope, should not replicate).
    client *out_scope_client = getMultiClient("SET {zho}E0 xxx", 1);
    out_scope_client->db = server.db[0];

    // Simulate MULTI/EXEC command
    simulateUnblockedMultiExec(out_scope_client);

    // Finish iterating the remaining keys; no more replication items expected.
    expectReadKeySequence(it, N_KEYS_PER_SLOT + 2, n_keys_to_read - 1);
    expectReadComplete(it);
    freeTestClient(in_scope_client);
    freeTestClient(out_scope_client);
}

TEST_F(BgIterationTestClusterSlots, modifyItems_MultipleMultiExecClients) {
    // This test ensures that command replication is properly handled when MULTI/EXEC
    // from different clients are interleaved.
    // This test is meant to replicate this bug https://issues.amazon.com/issues/ELMO-111294
    bgIterator *it = bgIteratorCreateSlotsIter("threeSlotIterator", BGITERATOR_FLAG_REPLICATION,
            slots_to_iterate, slots_to_iterate_size, NULL, iteratorCleanupFn, PRIVDATA);
    ...
    // MULTI command from out of scope client
    client *out_of_scope_client = static_cast<client*>(zcalloc(sizeof(client)));
    out_of_scope_client->cmd = lookupCommandByCString("multi");
    out_of_scope_client->flag.multi = 0;
    out_of_scope_client->db = server.db[0];
    
    // Simulate Multi command
    simulateUnblockedWrite(out_of_scope_client);
    
    // Populate post Multi Command execution state
    initClientMultiState(out_of_scope_client);
    out_of_scope_client->flag.multi = 1;

    // Now, Fake a modification to a key in slot 0 from another client (should replicate) 
    client *in_scope_client = getMultiClient("SET {06S}C0 xxx", 0);
    in_scope_client->db = server.db[0];

    // Simulate MULTI/EXEC command to trigger command replication
    simulateUnblockedMultiExec(in_scope_client);
    
    // Check that the commands replicated
    expectReadKey(it, N_KEYS_PER_SLOT + 1, nullptr);
    expectReadMultiReplication(it);
    expectReadReplication(it, in_scope_client);
    expectReadExecReplication(it);

    // Queue the remaining of the out-of-scope client's MULTI/EXEC block
    out_of_scope_client->mstate->cmd_flags |= CMD_WRITE;
    out_of_scope_client->cmd = lookupCommandByCString("SET");
    out_of_scope_client->argc = 3;
    out_of_scope_client->argv = static_cast<robj**>(zcalloc(sizeof(robj*) * out_of_scope_client->argc));
    out_of_scope_client->argv[0] = createStringObject("SET", strlen("SET"));
    out_of_scope_client->argv[1] = createStringObject("{zho}E0", strlen("{zho}E0"));
    out_of_scope_client->argv[2] = createStringObject("1", strlen("1"));
    queueMultiCommand(out_of_scope_client, CMD_WRITE);
    freeClientArgv(out_of_scope_client);

    // Add the Exec command
    out_of_scope_client->cmd = lookupCommandByCString("exec");
    out_of_scope_client->argc = 1;
    out_of_scope_client->argv = static_cast<robj**>(zcalloc(sizeof(robj*) * out_of_scope_client->argc));
    out_of_scope_client->argv[0] = createObject(OBJ_STRING, sdsnew("EXEC"));

    // Execute exec and the queued commands (not replicated)
    simulateUnblockedMultiExec(out_of_scope_client);

    // Finish iterating the remaining keys; no more replication items expected.
    expectReadKeySequence(it, N_KEYS_PER_SLOT + 2, n_keys_to_read - 1);
    expectReadComplete(it);
    freeTestClient(in_scope_client);
    freeTestClient(out_of_scope_client);
}
#endif
