...
tags "modules" {
    ...
    # AOF module aux tests
    # Config flags:
    # 1 << 0 = 1  - use aux_save2
    # 1 << 1 = 2  - before keyspace
    # 1 << 2 = 4  - after keyspace
    # 1 << 3 = 8  - no data
    # 1 << 4 = 16 - register AOF aux callbacks

    # 22 == 10110 - aux before and after keyspace with AOF callbacks (no aux_save2)
    # 23 == 10111 - aux_save2 before and after keyspace with AOF callbacks
    foreach test_case {22 23} {
    test "modules are able to persist globals via AOF rewrite (aof-use-rdb-preamble=no) test_case=$test_case" {
        set server_path [tmpdir "server.module-testrdb-aof"]
        start_server [list overrides [list loadmodule "$testmodule $test_case" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""] keep_persistence true] {
            r testrdb.set.before aof_before_val
            r testrdb.set.after aof_after_val
            assert_equal "aof_before_val" [r testrdb.get.before]
            assert_equal "aof_after_val" [r testrdb.get.after]
            r bgrewriteaof
            waitForBgrewriteaof r
        }
        # Restart server and verify aux data was loaded from AOF
        start_server [list overrides [list loadmodule "$testmodule $test_case" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""]] {
            assert_equal "aof_before_val" [r testrdb.get.before]
            assert_equal "aof_after_val" [r testrdb.get.after]
        }
    }
    ...
    # 20 == 10100 - aux after keyspace only with AOF callbacks
    test {modules are able to persist globals via AOF just after keyspace} {
        set server_path [tmpdir "server.module-testrdb-aof-after"]
        start_server [list overrides [list loadmodule "$testmodule 20" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""] keep_persistence true] {
            r testrdb.set.after aof_after_only
            assert_equal "aof_after_only" [r testrdb.get.after]
            r bgrewriteaof
            waitForBgrewriteaof r
        }
        start_server [list overrides [list loadmodule "$testmodule 20" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""]] {
            assert_equal "aof_after_only" [r testrdb.get.after]
        }
    }

    # Test that no MODULE AUXLOAD is written when there's no data
    # 22 == 10110 - AOF aux with before+after, but we don't set any data
    test {AOF module aux writes nothing when no data is set} {
        set server_path [tmpdir "server.module-testrdb-aof-nodata"]
        start_server [list overrides [list loadmodule "$testmodule 22" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""] keep_persistence true] {
            # Don't set any module aux data, just write a normal key
            r set mykey myvalue
            r bgrewriteaof
            waitForBgrewriteaof r
        }
        # Restart without module - should succeed because no MODULE AUXLOAD was written
    ...
    # Test that AOF module aux data coexists with key data
    # 22 == 10110 - AOF aux with before+after keyspace
    test {AOF module aux data coexists with regular keys} {
        set server_path [tmpdir "server.module-testrdb-aof-coexist"]
        start_server [list overrides [list loadmodule "$testmodule 22" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""] keep_persistence true] {
            r set normalkey normalvalue
            r set anotherkey anothervalue
            r testrdb.set.before aux_before
            r testrdb.set.after aux_after
            r bgrewriteaof
            waitForBgrewriteaof r
        }
        start_server [list overrides [list loadmodule "$testmodule 22" "dir" $server_path appendonly yes aof-use-rdb-preamble no save ""]] {
            assert_equal "normalvalue" [r get normalkey]
            assert_equal "anothervalue" [r get anotherkey]
            assert_equal "aux_before" [r testrdb.get.before]
            assert_equal "aux_after" [r testrdb.get.after]
        }
    }
}
